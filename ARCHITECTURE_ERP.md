...
**Reste à construire, dans l'ordre de dépendance du schéma** : Ventes & CRM Phase 3b, Rapports & BI, Administration (`erp_settings`) — chaque module rejoint `ERP_MODULES`/`NAV.erp` au moment où son écran existe réellement, jamais en avance (même discipline que la navigation ZegResto en son temps : pas de lien vers une page qui n'existe pas).
