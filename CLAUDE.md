...
- **Toute migration SQL est présentée pour relecture avant exécution — sans exception.** Chaque fichier `db/migrations/0NN_*.sql` est écrit et montré en clair, jamais lancé par CI ni exécuté à l'initiative de l'agent. Une fois qu'Anselme a explicitement validé une migration précise (ex. "vas-y", "exécute"), l'agent est autorisé à l'appliquer lui-même (outil MCP Supabase — `apply_migration`/`execute_sql`) au lieu de demander à Anselme de la coller manuellement dans le SQL Editor. Cette autorisation porte sur l'exécution, pas sur la relecture : chaque migration reste présentée et validée individuellement avant d'être appliquée, jamais en lot silencieux ni de façon anticipée sur des migrations pas encore montrées.
