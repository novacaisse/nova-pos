...
17 tables, toutes préfixées **`resto_`** (migrations 035-041) — même raison de collision que `hotel_` (une table `orders` nue entrerait en collision avec un futur usage générique du mot) :
...
| `resto_menu_categories`, `resto_menu_items` | Catégories et articles du menu (prix, description, temps de préparation, disponibilité). `station` existe sur `resto_menu_items` mais n'est **pas exploité en V1** — préparé pour un futur routage KDS multi-poste (grill/froid/pâtisserie…), pas de `resto_kitchen_stations`. |
...
| `resto_orders`, `resto_order_items` | Commande (table ou emporter/livraison, statut) et ses lignes (quantité, modificateurs choisis en JSON figé à l'ajout, prix unitaire figé). |
| `resto_kitchen_tickets` | Un ticket cuisine **par commande** (flux unique, pas de routage multi-poste en V1) — `en_attente` → `en_preparation` → `pret`. Mis à jour en **temps réel** (voir plus bas). |
| `resto_reservations` | Réservations client (staff **et** publiques, `source` distingue les deux) — voir la page publique plus bas. |
| `resto_recipes`, `resto_recipe_ingredients` | Recette optionnelle par article ; `ingredient_ref` référence **directement `public.products(id)`** — les ingrédients sont des produits ZegCaisse ordinaires, stock suivi par `stock_levels`/`stock_movements` déjà existants (même réutilisation que `hotel_pos-interne`). Pas de conversion d'unité automatique (limite assumée V1). |
| `resto_bills`, `resto_bill_splits`, `resto_bill_split_items`, `resto_bill_payments` | Note d'une commande, partage (`aucun`/`egal`/`detaille`), affectation ligne↔convive en mode détaillé, paiements (mobile money/espèces/carte). |
...
**Temps réel (Supabase Realtime) :** `resto_orders`, `resto_order_items` et `resto_kitchen_tickets` rejoignent la publication `supabase_realtime` (migration 038, `REPLICA IDENTITY FULL`) — première utilisation réelle de Supabase Realtime dans ce projet. `/app/resto/commandes` et `/app/resto/cuisine` s'abonnent via `postgres_changes` filtré par `organization_id`, avec un filet `refetchInterval` si la connexion websocket tombe. **`hotel_housekeeping_tasks` (ZegHotel), malgré un besoin similaire de statut à jour, reste en polling/invalidation TanStack Query classique — non converti.**

**RPC clés (toutes `security definer` sauf mention contraire, pattern de bridage RLS déjà établi pour ZegHotel) :**
- `add_resto_order_item()` : ajoute une ligne de commande, synchronise le ticket cuisine, décrémente le stock des ingrédients de la recette si elle existe (réutilise le trigger `apply_stock_movement()` et son garde-fou anti-survente) — `server` n'a aucun droit d'écriture direct sur `resto_order_items`.
...
- `add_resto_bill_payment()` : accumulation atomique sous verrou de ligne (même pattern que `add_sale_payment`) — dès que la somme des paiements couvre le total, la note passe `payee`, la commande `fermee`, la table redevient `libre`.
- `resto_public_organization_info()` / `resto_public_create_reservation()` : voir la page publique ci-dessous.

**Page publique (exception de scope validée) :** `/resto/reserver/$slug` est la **seule page publique de tout ZegOS** en dehors de `/`, `/tarifs`, `/inscription`, `/connexion`, `/souscription` — un formulaire de réservation client par restaurant (résolu par son `slug` d'organisation), volontairement autonome (pas de `PublicHeader`/`PublicFooter`, marketing ZegCaisse hors sujet ici). Il n'écrit **jamais** directement dans `resto_reservations` (aucune policy `insert to anon`) : il passe par `resto_public_create_reservation()`, seule porte d'écriture anonyme de tout ZegResto, qui valide (nom requis, couverts > 0, date dans le futur) et insère avec `statut = 'pending'`, `source = 'public'` — à confirmer par le staff dans `/app/resto/reservations`. Aucune limite de débit/anti-spam en V1 (assumé). Cette exception est **spécifique à ZegResto** et ne doit pas être étendue à ZegCaisse/ZegHotel sans validation explicite.

Routes : `/app/resto` (tableau de bord — CA/couverts/tables occupées/réservations en attente), `/app/resto/salle` (plan de salle interactif, glisser-déposer), `/app/resto/commandes` (prise de commande + facturation), `/app/resto/cuisine` (KDS temps réel), `/app/resto/menu` (catégories/articles/modificateurs/recettes), `/app/resto/reservations`, `/app/resto/rapports` (CA/ticket moyen/temps de service moyen/plats les plus vendus, sélecteur de période partagé), `/app/resto/equipe`, `/app/resto/parametres` (Restaurant/Zones). Même convention de fichiers plats que ZegHotel (`app.resto.tsx` + `app.resto.index.tsx` + `app.resto.<page>.tsx`).

Hors scope V1 (assumé, comme demandé) : intégrations livraison tierce (Uber Eats, etc.), réservation multi-restaurant depuis une même page publique, programme de fidélité, routage KDS multi-poste (`resto_menu_items.station` posé mais inexploité), conversion d'unité automatique dans les recettes, upload direct de photo d'article (URL texte seulement — aurait nécessité un nouveau bucket Supabase Storage), notifications automatisées dédiées ZegResto (pas de `notify_resto_*` sur le modèle de `notify_hotel_reservation_created` — non demandé pour ce chantier).
...
- ZegResto V1 : pas d'intégrations livraison tierce, pas de réservation multi-restaurant, pas de programme de fidélité, pas de routage KDS multi-poste, pas de conversion d'unité automatique dans les recettes, pas d'upload direct de photo d'article, pas de notifications automatisées dédiées (contrairement à ZegHotel — non demandé pour ce chantier).
