// Module Dépenses ZegHotel (mission "mise à jour ZegHotel", item 8 — "pareil
// à zegCaisse") — la table expenses et organization_settings sont déjà
// partagées au niveau base (pas de colonne app_module, cf. CLAUDE.md
// "convention de nommage" : seules les tables préfixées hotel_*/resto_* sont
// isolées par module) : cet écran réutilise donc verbatim les mêmes hooks
// que /app/depenses (ZegCaisse), seule la route change.
//
// Note permission : la policy expenses_insert exige
// has_module_permission(org, 'depenses', 'create') et permission_modules
// n'a une ligne 'depenses' que pour app_module='pos' (db/schema.sql) — comme
// pour 'produits'/'stock' déjà signalé dans RAPPORT_ZEGHOTEL_CORRECTIFS_2026-08.md,
// seul le rôle owner (court-circuité dans has_module_permission()) pourra
// réellement écrire tant que ce module n'est pas seedé aussi pour 'hotel'.
...
export const Route = createFileRoute("/app/hotel/depenses")({
  component: HotelDepensesPage,
});

function HotelDepensesPage() {
