// Module Fournisseurs ZegHotel (mission "Round 2 ZegHotel", scope confirmé
// par Anselme : nouveau module ZegHotel dédié réutilisant la table
// partagée). suppliers/purchase_orders/purchase_order_items ne sont PAS
// des tables préfixées hotel_* (contrairement à la convention CLAUDE.md
// pour les données propres à un module) — elles sont déjà partagées avec
// ZegCaisse, exactement comme products/categories (cf. /app/hotel/produits).
// Cet écran est donc un quasi miroir de /app/fournisseurs (ZegCaisse),
// mêmes hooks, seuls les rôles gérables changent (pas de rôle "stock" côté
// ZegHotel).
//
// Note permission (même classe de gap que Dépenses, cf. app.hotel.depenses.tsx) :
// permission_modules n'a une ligne 'fournisseurs' que pour app_module='pos'
// (open_view=false, db/schema.sql) — has_module_permission() ne filtre pas
// par app_module pour une clé donnée, mais sans ligne seedée pour les
// membres hôtel non-owner, seul le rôle owner (court-circuité dans
// has_module_permission()) pourra réellement lire/écrire tant que ce
// module n'est pas seedé aussi pour 'hotel'. Non corrigé ici (changement de
// policy/seed hors périmètre de cette phase sans confirmation explicite).
...
export const Route = createFileRoute("/app/hotel/fournisseurs")({
  component: HotelFournisseursPage,
});
...
function HotelFournisseursPage() {
  ...
  // Pas de rôle "stock" côté ZegHotel — owner/manager gèrent, front_desk/
  // housekeeping/accountant restent en lecture (si un jour seedés, cf.
  // note permission en tête de fichier).
  const canManage = myRole === "owner" || myRole === "manager";
  const canManagePO = myRole === "owner" || myRole === "manager";
