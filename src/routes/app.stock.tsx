...
import { PRODUCTS as SEED, formatXOF, type Product } from "@/lib/mock/catalog";
import { STOCK_MOVES as SEED_MOVES, type StockMove, type StockMoveType } from "@/lib/mock/stock";
...
type ProductWithThreshold = Product & { min_stock: number };

function StockPage() {
  const [products, setProducts] = useState<ProductWithThreshold[]>(SEED.map((p) => ({ ...p, min_stock: 25 })));
  ...
  const [editingThreshold, setEditingThreshold] = useState<ProductWithThreshold | null>(null);
  ...
  const outOfStock = products.filter((p) => p.stock < p.min_stock).sort((a, b) => a.stock - b.stock);

  const addMove = (m: StockMove) => {
    setMoves((l) => [m, ...l]);
    setProducts((l) => l.map((p) => (p.id === m.product_id ? { ...p, stock: Math.max(0, p.stock + m.qty) } : p)));
  };

  const updateThreshold = (id: string, min: number) =>
    setProducts((l) => l.map((p) => (p.id === id ? { ...p, min_stock: min } : p)));

  return (
    <div>
      <PageHeader title="Stock" subtitle="Suivi des mouvements et alertes"
        actions={<button onClick={() => setShowAdjust(true)} className="flex items-center gap-2 rounded-xl bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground shadow-elegant hover:opacity-90"><Plus className="h-4 w-4" /> Nouveau mouvement</button>}
      />
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-muted/40">
                      <tr className="text-left text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                        ...
                        <th className="px-4 py-3">Motif</th><th className="px-4 py-3">Utilisateur</th>
                    ...
                    {products.map((p) => {
                      ...
                      return (
                        <tr key={p.id} className="border-t border-border/60 hover:bg-muted/40">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2"><span className="text-lg">{p.emoji}</span><span className="font-medium">{p.name}</span></div>
                          </td>
                          <td className={cn("tabular px-4 py-3 text-right font-bold", low && "text-warning-foreground")}>{p.stock}</td>
                          <td className="tabular px-4 py-3 text-right text-muted-foreground">{p.min_stock}</td>
                          <td className="px-4 py-3">
      ...
      <AnimatePresence>
        {showAdjust && <AdjustDialog products={products} onClose={() => setShowAdjust(false)} onSave={(m) => { addMove(m); setShowAdjust(false); }} />}
        {editingThreshold && (
          <ThresholdDialog product={editingThreshold} onClose={() => setEditingThreshold(null)}
            onSave={(v) => { updateThreshold(editingThreshold.id, v); setEditingThreshold(null); }} />
        )}
      </AnimatePresence>
    </div>
  );
...
function AdjustDialog({ products, onClose, onSave }: { products: ProductWithThreshold[]; onClose: () => void; onSave: (m: StockMove) => void }) {
  const [productId, setProductId] = useState(products[0].id);
  const [type, setType] = useState<StockMoveType>("ajustement");
  const [qty, setQty] = useState(0);
  const [reason, setReason] = useState("");

  const p = products.find((x) => x.id === productId)!;
  const signed = type === "entree" ? Math.abs(qty) : type === "sortie" || type === "perte" || type === "transfert" ? -Math.abs(qty) : qty;

  const submit = () => {
    if (!qty || !reason.trim()) return;
    onSave({
      id: `m-${Date.now()}`, product_id: p.id, product_name: p.name,
      type, qty: signed, reason, user: "Aïcha K.", created_at: new Date().toISOString(),
    });
  };
    ...
    <Overlay onClose={onClose}>
      ...
      <div className="space-y-3 p-5">
        ...
        <div className="grid grid-cols-5 gap-1">
          {(Object.keys(TYPE_META) as StockMoveType[]).map((t) => (
            <button key={t} onClick={() => setType(t)}
              className={cn("rounded-lg border py-2 text-[11px] font-semibold", type === t ? "border-primary bg-primary/10 text-primary" : "border-border")}>
              {TYPE_META[t].label}
            </button>
          ))}
        </div>
        <label className="block"><div className="mb-1 text-xs font-semibold text-muted-foreground">Quantité (signée pour ajustement)</div>
        ...
        <div className="rounded-xl bg-muted p-3 text-xs">
          Impact sur le stock : <b className={signed >= 0 ? "text-success" : "text-destructive"}>{signed > 0 ? "+" : ""}{signed}</b> · Nouveau stock : <b>{Math.max(0, p.stock + signed)}</b>
        </div>
...
function ThresholdDialog({ product, onClose, onSave }: { product: ProductWithThreshold; onClose: () => void; onSave: (v: number) => void }) {
  const [v, setV] = useState(product.min_stock);
  return (
