...
//
// ZegCaisse et ZegHotel sont réellement fonctionnels aujourd'hui, chacun
// avec son propre formulaire de configuration (pas de champs POS-only
// comme "secteur" dans celui de ZegHotel). ZegERP reste "à venir".
import { useState } from "react";
import { motion } from "framer-motion";
import { ArrowLeft, ArrowRight, AlertCircle, BedDouble, Boxes, Clock, Image as ImageIcon, Loader2, LogOut, Store } from "lucide-react";
...
export function OnboardingFlow({ onSignOut }: { onSignOut: () => Promise<void> }) {
  const [step, setStep] = useState<"pick-app" | "setup-pos" | "setup-hotel">("pick-app");
  ...
  return (
    <AppPicker
      onPick={(key) => setStep(key === "hotel" ? "setup-hotel" : "setup-pos")}
      onSignOut={onSignOut}
    />
  );
}
