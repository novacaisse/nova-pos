...
export function OnboardingFlow({ onSignOut }: { onSignOut: () => Promise<void> }) {
  const [step, setStep] = useState<"pick-app" | "setup-pos" | "setup-hotel" | "setup-resto">("pick-app");
  ...
  return (
    <AppPicker
      onPick={(key) => setStep(key === "hotel" ? "setup-hotel" : key === "resto" ? "setup-resto" : "setup-pos")}
      onSignOut={onSignOut}
    />
  );
}
