  ...
  if (!currentOrganization) {
    return (
      <div className="flex items-center gap-2 rounded-xl border border-dashed border-border bg-card px-3 py-2 text-xs text-muted-foreground">
        <Store className="h-4 w-4" /> Aucune organisation
      </div>
    );
  }
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          <div className="min-w-0 leading-tight">
            <div className="truncate text-[11px] font-medium uppercase tracking-wider text-muted-foreground">Organisation</div>
      ...
      <PopoverContent align="start" className="w-72 p-1.5">
        ...
        <div className="mt-1 border-t pt-1">
          <Link to="/app/parametres" search={{ openAdd: true }} onClick={() => setOpen(false)}
