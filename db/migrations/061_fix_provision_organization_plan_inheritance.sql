-- Migration 061 — Corrige provision_organization() : une organisation
-- supplémentaire créée sous un compte déjà abonné (plan payé actif pour
-- cette app_module) démarrait TOUJOURS en 'trial' avec un nouveau délai de
-- 3 jours, quel que soit l'état réel de account_subscriptions. Symptôme
-- rapporté : un compte au plan Pro qui crée une 2e boutique la voit
-- afficher "Essai gratuit" au lieu du plan Pro (bannière d'essai, mode
-- lecture seule potentiel via useReadOnlyMode, organizations.plan lu
-- directement par TrialBanner/useAppNotifications/app.parametres.tsx —
-- src/lib/trial.ts ne regarde que organizations.plan/trial_ends_at, jamais
-- account_subscriptions). AddOrganizationDialog.tsx annonçait déjà le
-- comportement correct ("En période d'essai tant qu'aucun abonnement actif
-- n'existe encore pour cette application sur ce compte") — jamais implémenté
-- dans provision_organization(). Présentée pour relecture — NE PAS exécuter
-- automatiquement.
--
-- Corrige aussi une faille adjacente : une organisation supplémentaire
-- créée PENDANT l'essai gratuit repartait sur un compte à rebours de 3
-- jours entièrement nouveau plutôt que de partager la même échéance que le
-- compte — un propriétaire aurait pu prolonger indéfiniment son essai en
-- créant une nouvelle boutique juste avant l'expiration.
--
-- Signature strictement inchangée (7 arguments text, mêmes noms/types/
-- défauts) : create or replace function remplace en toute sécurité, pas de
-- drop nécessaire (voir CLAUDE.md, pièges Postgres récurrents).
...
declare
  ...
  -- État réel de l'abonnement du compte pour cette app_module (s'il existe
  -- déjà) — la nouvelle organisation doit hériter de cet état plutôt que de
  -- démarrer un essai indépendant, sauf s'il n'existe encore aucun abonnement
  -- pour ce couple (account_id, app_module), auquel cas elle en est le point
  -- de départ (comportement d'origine, inchangé).
...
begin
  ...
  -- Ne crée account_subscriptions que s'il n'existait pas déjà (`do
  -- nothing`) — dans tous les autres cas la ligne existante (active ou
  -- trialing) reste celle dont hérite la nouvelle organisation ci-dessus,
  -- jamais réécrite ici.
